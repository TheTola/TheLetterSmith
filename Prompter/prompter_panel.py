...
# - Five extra text areas: one global, four per-image (all PlainTextEdits)
# - Helpful options produce a Guidance: block for each image prompt (inserted after Quality Standard)
# - On each Generate: randomly choose values from list files for role, order, effort, format (if present),
#   but DO NOT randomize type, color, or topic (these are user-chosen).
...
def _pick_random_nonempty_line(name: str) -> Tuple[Optional[str], Optional[Path]]:
    ...
    cleaned = [c for c in cleaned if c]  # remove empties after cleaning
    if not cleaned:
...
def assemble_prompt_for_image(subject: str,
    ...
    # Build subject core (no trailing dot). Use lowercase so it reads as the sentence ender.
    subject_core = ""
    if subject:
    ...
    # Role + combined fragment paragraph
    ...
    # Type & color as separate paragraph(s)
    if color_choice:
    ...
    # Quality standard (effort)
    if effort_line:
        paragraphs.append(f"When making this image, {effort_line.strip()}")

    # Guidance insertion (always insert if present, after effort)
    if guidance:
...
class PromptWriterPanel(QtWidgets.QWidget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        # track last focused preview widget (for Copy Prompt)
        ...
        # keep references to per-block copy buttons if needed later
        self._per_copy_buttons: Dict[str, QtWidgets.QPushButton] = {}

        # visionary pulse state
        self._visionary_timer: Optional[QtCore.QTimer] = None
        self._visionary_colors = [QtGui.QColor("#0a46ff"), QtGui.QColor("#00e5ff"), QtGui.QColor("#ffffff"), QtGui.QColor("#00e5ff")]
        self._visionary_index = 0
        self._visionary_effect: Optional[QtWidgets.QGraphicsDropShadowEffect] = None
        ...
        # start the animated glow on "The Visionary"
    ...
    def _build_ui(self):
        ...
        self.btn_copy = QtWidgets.QPushButton("Copy All Prompts")
        ...
        # --- The Visionary (animated, glowing link) — placed NEXT TO "Erase All"
        self.lbl_best_results = QtWidgets.QLabel("For best results, use ")
        self.lbl_best_results.setObjectName("lbl_best_results")
        ...
        self.btn_visionary.setToolTip("Open The Visionary (ChatGPT)")

        # subtle separator dot
        self._sep_dot = QtWidgets.QLabel("•")
        self._sep_dot.setObjectName("sep_dot")
        self._sep_dot.setFixedWidth(10)
        self._sep_dot.setAlignment(Qt.AlignCenter)

        header.addWidget(self._sep_dot)
        header.addWidget(self.lbl_best_results)
        ...
        # Main: left selectors + helpful + extras ; right = stacked preview windows
        ...
        # populate type combo according to spec:
        # - lines starting with '- ' or '• ' are selectable options
        # - non-bullet non-empty lines are header rows (disabled + bold)
        ...
        # Right column: stacked "windows" (label + read-only editor + per-area copy) inside a scroll area
        ...
        # create four "window" style preview blocks
        for img in self._images:
            # header row: label (left) + copy button (right)
            header_row = QtWidgets.QWidget()
            hr_layout = QtWidgets.QHBoxLayout(header_row)
            ...
            hr_layout.addWidget(header_label)
            hr_layout.addStretch(1)
            hr_layout.addWidget(copy_btn)
            bl.addWidget(header_row)
            bl.addWidget(editor)
            self._preview_layout.addWidget(block)

            # store references
            self.preview_widgets[img] = editor
            self._per_copy_buttons[img] = copy_btn

            # connect per-area copy
            copy_btn.clicked.connect(lambda _, key=img: self._copy_single_prompt(key))
    ...
    def _apply_styles(self):
        ...
        QLabel { color: #dcdce0; }
        QLabel#lbl_best_results { color: #aab0b8; }
        QLabel#sep_dot { color: #3a3f4a; }
    ...
    # -----------------------
    # Visionary link — open + glow pulse
    # -----------------------
    def _open_visionary(self):
        try:
            QtGui.QDesktopServices.openUrl(QUrl(self.VISIONARY_URL))
        except Exception:
            # silent-safe: if shell open fails, do nothing
            pass

    def _start_visionary_pulse(self):
        # soft glow behind the button text
        self._visionary_effect = QtWidgets.QGraphicsDropShadowEffect(self.btn_visionary)
        self._visionary_effect.setOffset(0, 0)
        self._visionary_effect.setBlurRadius(18)
        self._visionary_effect.setColor(self._visionary_colors[0])
        self.btn_visionary.setGraphicsEffect(self._visionary_effect)

        # timer to rotate colors (deep blue → cyan → white → cyan ...)
        self._visionary_timer = QtCore.QTimer(self)
        self._visionary_timer.setInterval(480)  # ~2 pulses per second
    ...
    def _tick_visionary_pulse(self):
        # rotate the glow color
        self._visionary_index = (self._visionary_index + 1) % len(self._visionary_colors)
        col = self._visionary_colors[self._visionary_index]

        # update the halo effect
        if self._visionary_effect:
            self._visionary_effect.setColor(col)

        # IMPORTANT: avoid f-strings here so literal '}' don't break parsing
        style = (
                    "QPushButton#visionary_btn{"
                    "background:transparent;border:none;padding:0;margin-left:2px;"
                    "color:%s;font-weight:700;text-decoration:underline;}"
                    "QPushButton#visionary_btn:hover{color:#ffffff;}"
                ) % col.name()

        # apply per-widget stylesheet (keeps your panel-level styles intact)
    ...
    def _on_refresh(self):
        ...
        # re-populate type combo (now following spec: bullets selectable; non-bullet headers disabled)
        self.cmb_type.clear()
        self.cmb_type.addItem("— none —")
        types, _ = _read_list_file_cached("type.txt")
        for t in types:
    ...
    def _on_generate(self):
        ...
        per_extras = {
            "cover.png": self.txt_cover.toPlainText().strip(),
            "letter.png": self.txt_letter.toPlainText().strip(),
            "wall.png": self.txt_wall.toPlainText().strip(),
            "back.png": self.txt_back.toPlainText().strip()
        }
    ...
    def _copy_single_prompt(self, key: str):
        """Copy one prompt (by key) to clipboard."""
    ...
    def _copy_all_prompts(self):
        """Copy all prompts in order to clipboard with headings."""
        parts: List[str] = []
        # Use consistent order via self._images
        for img in self._images:
            w = self.preview_widgets.get(img)
            content = w.toPlainText() if w else ""
            parts.append(f"{img}:\n{content}")
        final = "\n\n" + "\n\n".join(parts) if parts else ""
        QtWidgets.QApplication.clipboard().setText(final.strip())

    def _copy_current_prompt(self):
        # retained for compatibility: copy last-focused or first
        src = None
    ...
    def popup(self):
        ...
        w = min(int(screen.width() * 0.72), 980)
        h = min(int(screen.height() * 0.86), 900)
        target = QtCore.QRect(40, 40, w, h)
        off = QtCore.QRect(-w, 40, w, h)
